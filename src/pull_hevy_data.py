"""
Heavy Workout Data Extractor - Refactored with Sync Manager
Extracts workout data from Heavy API and stores it in PostgreSQL database.
"""
...
class HevyWorkoutData(BaseModel):
    """Pydantic model for Heavy workout data."""
    ...
    def from_api_response(cls, workout_data: dict) -> list["HevyWorkoutData"]:
        """Convert Heavy API workout response to HevyWorkoutData instances."""
        workout_sets = []
...
class HevyAPIClient:
    ...
    def __init__(self, api_key: str):
        ...
        self.base_url = "https://api.heavy.com/v1"
        self.headers = {"api-key": api_key}

    def get_workouts(self) -> list[dict]:
        """Fetch all workouts from Heavy API with pagination."""
...
def sync_hevy_data_for_user(user_id: int) -> dict:
    """
    Main function to sync Heavy workout data for a user using the sync manager.
    ...
    try:
        with get_db_session_context() as db:
            ...
            if not creds or not creds.encrypted_hevy_api_key:
                return {"error": "No Heavy API key found for user"}
    ...
    try:
        ...
        logger.info(f"Heavy sync completed for user {user_id}: {summary}")
        return summary

    except Exception as e:
        error_msg = f"Failed to sync Heavy data for user {user_id}: {str(e)}"
        logger.error(error_msg)
        return {"error": error_msg, "user_id": user_id}


def get_hevy_sync_status(user_id: int) -> dict[str, Any]:
    """Get sync status for Heavy data."""
    return get_sync_statistics(user_id, source="heavy")  # type: ignore[no-any-return]
