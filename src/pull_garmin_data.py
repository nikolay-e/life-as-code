...
from models import HRV, BodyBattery, HeartRate, Sleep, Steps, Stress, Weight
    ...
    try:
        ...
        sync_configs = [
            ...
            },
            {
                ...
                "api_method": "get_heart_rate_data",
            ...
            },
            {
                ...
                "parser_class": GarminBodyBatteryData,
                "model_class": BodyBattery,
                "unique_fields": ["date"],
                "source": "garmin",
                "api_args": {"date_range": (start_date, end_date)},
            },
...
class GarminAPIWrapper:
    ...
    def get_weight_data(self, date_range: tuple) -> list[dict]:
        """Get weight and body composition data for date range."""
        results = []
        start_date, end_date = date_range
        ...
        while current_date <= end_date:
            try:
                date_str = current_date.strftime("%Y-%m-%d")
                weight_data = self.api.get_weigh_ins(date_str)
                ...
                logger.warning(f"Failed to get weight data for {current_date}: {e}")

            current_date += datetime.timedelta(days=1)

        return results

    def get_heart_rate_data(self, date_range: tuple) -> list[dict]:
        ...
        while current_date <= end_date:
            try:
                ...
                if hr_data:
                    hr_data["date"] = current_date
                    results.append(hr_data)

            except Exception as e:
                logger.warning(f"Failed to get heart rate data for {current_date}: {e}")

            current_date += datetime.timedelta(days=1)

        return results

    def get_body_battery(self, date_range: tuple) -> list[dict]:
        ...
        while current_date <= end_date:
            try:
                date_str = current_date.strftime("%Y-%m-%d")
                bb_data = self.api.get_body_battery(date_str)

                if bb_data:
                    bb_data["date"] = current_date
                    results.append(bb_data)

            except Exception as e:
                logger.warning(f"Failed to get body battery for {current_date}: {e}")
