...
class GarminBodyBatteryData(GarminBaseModel):
    """Pydantic model for Garmin Body Battery data."""

    charged: int | None = Field(None, description="Energy charged")
    drained: int | None = Field(None, description="Energy drained")
    highest: int | None = Field(None, description="Highest battery level")
    lowest: int | None = Field(None, description="Lowest battery level")

    @classmethod
    def get_field_mappings(cls) -> dict[str, list[str]]:
        return {
            "charged": ["charged", "bodyBatteryChargedValue", "energyCharged"],
            "drained": ["drained", "bodyBatteryDrainedValue", "energyDrained"],
            "highest": ["highest", "bodyBatteryHighestValue", "maxValue"],
            "lowest": ["lowest", "bodyBatteryLowestValue", "minValue"],
        }
