...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "(hrv >= 0 AND hrv <= 500) OR hrv IS NULL", name="valid_eight_sleep_hrv"
        ),
        sa.CheckConstraint(
    ...
    with op.batch_alter_table("user_credentials", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("eight_sleep_email", sa.String(length=200), nullable=True)
        )
        batch_op.add_column(
            sa.Column(
                "encrypted_eight_sleep_password", sa.String(length=500), nullable=True
            )
        )
        batch_op.add_column(
            sa.Column(
                "encrypted_eight_sleep_access_token",
                sa.String(length=1000),
                nullable=True,
            )
        )
        batch_op.add_column(
            sa.Column("eight_sleep_token_expires_at", sa.DateTime(), nullable=True)
        )
...
def downgrade() -> None:
    ...
    op.create_check_constraint(
        "valid_sync_source",
        "data_sync",
        "source IN ('garmin', 'hevy', 'whoop', 'google', 'apple_health')",
    )
