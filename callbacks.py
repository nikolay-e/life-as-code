"""
Dash callbacks for Life-as-Code application.
Contains all callback functions organized by functionality.
"""
...
def create_figure(data, traces, layout_kwargs):
    """Factory function to create standardized Plotly figures."""
    fig = go.Figure()
    for trace in traces:
...
def csrf_protected_callback(func):
    """Decorator for CSRF protection in callbacks."""
...
def register_callbacks(app):
    ...
    def sync_data(garmin_clicks, hevy_clicks):
        if not any([garmin_clicks, hevy_clicks]):
            return ""
                ...
                if "garmin" in button_id:
                    ...
                    def run_hevy_sync():
                        try:
                            ...
                            logger.info(
                                f"Hevy sync completed for user {user_id}: {result}"
                            )
                        except Exception as e:
                            logger.error(f"Hevy sync failed for user {user_id}: {e}")
                    ...
                    return (
                        "🏋️‍♂️ Hevy sync started... (Check Data Status tab for progress)"
                    )
...
def create_energy_chart(energy_df):
    """Create energy expenditure chart."""
    if energy_df.empty:
        fig = go.Figure()
...
def create_steps_chart(steps_df):
    """Create steps and distance chart."""
    if steps_df.empty:
        fig = go.Figure()
